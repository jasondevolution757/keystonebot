...
var depleteKey = function(user, alt, context) { 
	...
	connect.query('UPDATE keystones SET status = "Depleted", updated_at = NOW() WHERE user = ? AND alt = ?', [user, alt], function(error, results) { 
...
var displayKeys = function(context) { 
	...
	connection.query('SELECT * FROM keystons WHERE status IN ("Active", "Depleted")', function(error, results) { 
		if (error) throw error;
		connection.end();

...
module.exports = new Clapp.Command({
    ...
    fn: (argv, context) => {
        ...
        setKeystone(user, alt, dungeon, level, guildId, context);
        return "Keystone set!\n"
    },
    args: [
        {
...
var addKeyToDataStore = function(user, alt, dungeon, level, status, guildId, context) {
    ...
    connection.query('SELECT * FROM keystones WHERE user = ? AND alt = ? AND status IN ("Active", "Depleted") AND guild_id = ?', [user, alt, guildId], function(error, results, fields) {
        ...
        if (results.length == 1) {
            connection.query('UPDATE keystones SET dungeon = ?, level = ?, updated_at = NOW()  WHERE user = ? AND alt = ? AND status IN ("Active", "Depleted") AMD guild_id = ?', [dungeon, level, user, alt, guildId], function(error, results, fields) {
                if (error) throw error;
                connection.end();

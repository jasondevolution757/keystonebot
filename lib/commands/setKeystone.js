...
var getDungeon = function(dungeon) {
    if (dungeon == '') {
        ...
        var shrotAltRe = new RegExp('ku', 'i');
        if (longRe.exec(dungeon) || shortRe.exec(dungeon) || shortAltRe.exec(dungeon)) {
...
var addKeyToDataStore = function(user, alt, dungeon, level, status, context) {
    ...
    connection.query('SELECT * FROM keystones WHERE user = ? AND alt = ? AND status IN ("Active", "Depleted")', [user, alt], function(error, results, fields) {
        ...
        if (results.length == 1) {
            connection.query('UPDATE keystones SET dungeon = ?, level = ?, updated_at = NOW()  WHERE user = ? AND alt = ? AND status IN ("Active", "Depleted")', [dungeon, level, user, alt], function(error, results, fields) {
                if (error) throw error;
                connection.end();
                displayKeys(context);
            });
...
var displayKeys = function(context) {
    ...
    connection.query('SELECT * FROM keystones WHERE status IN ("Active", "Depleted") ORDER BY level DESC, dungeon', function(error, results) {
        if (error) throw error;
        connection.end();
